...
const SearchForm = () => {
    ...
    return (
        <form className="params" onSubmit={handleSubmit}>
                <label>
                    <div className="input">
                        <input  type="search" name="search" placeholder="find your book..."  required className="search" value={values.search} ref={input} onChange={handleChange} />
                        <input type='submit' value='Search' className="button" disabled={isSubmitting} />
                    </div>
                </label>
                ...
                <label>
                    <div className="categoriesContainer">
                      <span className="ctegories-name">Categories:</span> 
                       <select name="categories" className="categories" value={values.categories} onChange={handleChange}>
                           {optionCategories.map((catigorie) => {
                               if (catigorie === 'all') {
                                   return (
                                    <option key={_.uniqueId()} value={catigorie} defaultValue>{catigorie}</option>
                                   );
                               }
                               return (
                                <option key={_.uniqueId()} value={catigorie}>{catigorie}</option>
                               )
                           })}
                        </select>
                    </div>
